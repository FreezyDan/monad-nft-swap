...
import { fetchNftsForAddress, ownsToken } from "../lib/nft";
...
export default function SelectNFT(): JSX.Element {
  ...
  async function handleContinue() {
    ...
    setVerifying(true);
    try {
      const chosen = nfts.filter((it) => selected[`${it.contract}:${it.tokenId}`]);
      if (chosen.length === 0) return;

      // verify still owned by counterparty
      const checks = await Promise.all(
        chosen.map((it) => ownsToken(owner as `0x${string}`, it.contract, it.tokenId))
      );
      const failures = chosen.filter((_, i) => !checks[i]);
      if (failures.length > 0) {
        const list = failures.map((f) => `${f.collectionName} #${f.tokenId}`).join("\n");
        alert("Some selected NFTs are not currently owned by the address:\n\n" + list);
        return;
      }

      // go to BuildSwap with selections & counterparty
      navigate("/swap/build", {
        state: {
          counterparty: owner,
          theirSelections: chosen, // array of {contract, tokenId, ...}
        },
      });
    } finally {
      setVerifying(false);
    ...
    <div className="swap-page">
      ...
      <section style={{ padding: "24px 16px 0" }}>
        <div style={{ maxWidth: 980, margin: "0 auto" }}>
          <div style={{ border: "2px dashed rgba(255,255,255,0.2)", borderRadius: 12, padding: 16, textAlign: "center" }}>
            <div style={{ color: "rgba(255,255,255,0.75)", fontWeight: 700, marginBottom: 12 }}>
              {selectedCount} NFT(s) Selected.
            </div>

            <button
              disabled={selectedCount === 0 || verifying}
              onClick={selectedCount > 0 ? handleContinue : undefined}
              className={selectedCount === 0 || verifying ? "" : "cta-primary"}
              style={
                selectedCount === 0 || verifying
                  ? { height: 56, padding: "0 28px", borderRadius: 28, fontWeight: 800, fontSize: 18,
                      background: DISABLED_GRADIENT, color: "rgba(255,255,255,0.7)", boxShadow: "0 10px 20px rgba(0,0,0,0.45)", border: "1px solid rgba(255,255,255,0.08)", cursor: "not-allowed" }
                  : { height: 56, padding: "0 28px", borderRadius: 28, fontWeight: 800, fontSize: 18 }
              }
            >
              {selectedCount === 0 ? "SELECT YOUR NFT" : verifying ? "VERIFYING…" : "CONTINUE"}
            </button>

            <div style={{ marginTop: 12, color: "rgba(255,255,255,0.6)", fontSize: 12 }}>
              Browsing wallet: <span style={{ fontFamily: "monospace" }}>{owner ?? "(use 0x address)"}</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: "24px 16px 40px" }}>
        <div style={{ maxWidth: 980, margin: "0 auto" }}>
          ...
          {!owner ? (
            <div style={{ color: "rgba(255,255,255,0.7)" }}>Please use a 0x wallet address.</div>
          ) : loading ? (
            <div style={{ color: "rgba(255,255,255,0.7)" }}>Loading NFTs…</div>
          ) : byCollection.length === 0 ? (
            <div style={{ color: "rgba(255,255,255,0.7)" }}>No NFTs found in verified collections.</div>
          ) : (
            byCollection.map(([coll, items]) => (
                    ...
                    return (
                      <button key={key} onClick={() => toggle(key)}
                        ...
                        {nft.image ? (
                          ...
                          <div style={{ width: "100%", height: 160, display: "grid", placeItems: "center", color: "rgba(255,255,255,0.6)", fontSize: 12 }}>
                            No Image
                          </div>
                        )}
          ...
          <div style={{ display: "flex", justifyContent: "center", marginTop: 20 }}>
            <Link to="/swap/new" className="cta-secondary" style={{ textDecoration: "none", height: 56, borderRadius: 28, padding: "0 24px", fontSize: "1.05rem", fontWeight: 800 }}>
              {"\u2190"} Back
            </Link>
          </div>
        </div>
